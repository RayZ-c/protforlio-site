 ...
 * ICON SET
 * ---------------------------------------------------------------------------
 * Geometry is Lucide (ISC licensed — see /THIRD_PARTY_NOTICES.md), inlined
 * rather than installed from a package for three reasons:
 *
 *   1. no runtime dependency and no extra request — the paths ride along in a
 *      bundle the page already downloads,
 *   2. `stroke="currentColor"` makes every icon inherit the panel's text
 *      colour, including its hover and open states, for free,
 *   3. each icon's child elements stay individually addressable, so the
 *      animations in icons.css can move ONE part — a cog's ring, a stack's top
 *      layer, the far half of a link — instead of transforming the whole glyph.
 *
 * The motion is modelled on lucide-animated.com and heroicons-animated.com but
 * rewritten as CSS keyframes: both of those are React + Motion, and this site
 * adds no dependencies.
 *
 * `ICON_FAMILY` groups icons that read as the same kind of object onto one
 * keyframe set, so the vocabulary stays small and deliberate instead of
 * becoming 46 unrelated effects. Anything unlisted falls back to `nudge`.
 */

export const ICON_FAMILY = {
  'arrow-down-to-line': 'drop',
  'arrow-up': 'lift',
  'award': 'lift',
  'blocks': 'stack',
  'boxes': 'stack',
